...
def is_valid_coordinate111(latitude: str, longitude: str) -> bool:
    try:
...
def display_weather_info(weather_info: str, user_window: tk.Toplevel) -> None:
    ...
    url = f'https://api.openweathermap.org/data/2.5/weather?lat={66}&lon={77}&appid={API_KEY}'
    response = requests.get(url)
    data = response.json()
